...
import com.example.affablebeanui.service.CardService;
...
public class ProductController {
    ...
    public String viewCart(Model model){
        model.addAttribute("cartItems",cardService.getAllProducts());
    ...
    public String checkout(Product product){
        ...
        int i=0;
        for (Product cartItem:cardService.getAllProducts()){
            cartItem.setQuantity(product.getQuantityList().get(i));
            i++;
        }
        cardService.getAllProducts()
                .forEach(System.out::println);
        return "redirect:/ui/checkout-view";
    }
    @ModelAttribute("total")
    public double totalAmount(){
        return cardService.getAllProducts()
                .stream()
    ...
    public List<CartItem> cartItems(){
        return cardService.getAllProducts().stream()
                .map(p -> new CartItem(
                p.getId(),
                p.getName(),
                p.getPrice(),
